...
// ************************************************************************
/*
void Tetrahedral::Ler_geometria(int nel,const Vertice * vert,FILE * finput,
				int map[],
				int sgn[],int& NG,int& NL,EDGE border[],int Ng[],
				int & NF, int Face[], int Fng[],int f_mask[])
{
  int N[4],sgn0,ng0,imin,imax;
  // Leitura do arquivo de dados
#ifdef PRINTF_ON
  printf("Tetrahedral::Ler\n");
#endif
  fscanf(finput,"%*d %*d %*d %d %d %d %d",&N[0], &N[1], &N[2], &N[3]);
  // reduz os valores dos nos de 1;
  N[0]--;N[1]--;N[2]--;N[3]--;
#ifdef PRINTF_ON
  printf("Antes de ordenar : %d %d %d %d\n",N[0],N[1],N[2],N[3]);
#endif
  ordenar4(N,f_mask); //ordenar os nos
#ifdef PRINTF_ON  
  printf("Apos ordenar : %d %d %d %d\n",N[0],N[1],N[2],N[3]);
#endif  
  // **********************************************************************
  // map e sgn dos nos
  // **********************************************************************
  ...
  sgn[3]=1;
  // Primeira aresta AB
  teste_aresta(P[0],N[0],N[1],sgn0,ng0,NG,NL,border,Ng,nel,0,vert);
  imin=4;
  imax=imin+P[0]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Segunda aresta BC
  teste_aresta(P[1],N[1],N[2],sgn0,ng0,NG,NL,border,Ng,nel,1);
  imin=imax+1;
  imax=imin+P[1]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta AC
  teste_aresta(P[1],N[0],N[2],sgn0,ng0,NG,NL,border,Ng,nel,2);
  imin=imax+1;
  imax=imin+P[1]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta AD
  teste_aresta(P[2],N[0],N[3],sgn0,ng0,NG,NL,border,Ng,nel,3);
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta BD
  teste_aresta(P[2],N[1],N[3],sgn0,ng0,NG,NL,border,Ng,nel,4);
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta CD
  teste_aresta(P[2],N[2],N[3],sgn0,ng0,NG,NL,border,Ng,nel,5);
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  imin=imax+1;
  // Face ABC
  teste_face(N[0],N[1],N[2],sgn0,ng0,NG,NF,Face,Fng);
  map_face(imin,P[0],P[1],sgn0,ng0,map,sgn);
  // Face ABD
  teste_face(N[0],N[1],N[3],sgn0,ng0,NG,NF,Face,Fng);
  map_face(imin,P[0],P[2],sgn0,ng0,map,sgn);
  // Face BCD
  teste_face(N[1],N[2],N[3],sgn0,ng0,NG,NF,Face,Fng);
  map_face(imin,P[1],P[2],sgn0,ng0,map,sgn);
  // Face ACD
  teste_face(N[0],N[2],N[3],sgn0,ng0,NG,NF,Face,Fng);
  map_face(imin,P[1],P[2],sgn0,ng0,map,sgn);
};
*/
// ************************************************************************
// ************************************************************************
/*
void Tetrahedral::Processar_geometria(int nel,
				      const Vertice * vert,
				      const int numv,
				      const int * VN,
...
{
 if(numv != 4){
    printf("Incompatibilidade: Dado do elemento nao eh de Tetraedro\n");
    exit(0);
  }
  int N[4],sgn0,ng0,imin,imax;
#ifdef PRINTF_ON
  printf("Tetrahedral::Ler\n");
#endif
  // recebe os numeros dos vertices locais
  for(int i=0;i<4;i++)N[i]=VN[i];
  ...
  sgn[3]=1;
  // Primeira aresta AB
  teste_aresta(P[0],N[0],N[1],sgn0,ng0,NG,NL,border,Ng,nel,0,vert,0);
  sinal[0]=sgn0;
  imin=4;
  imax=imin+P[0]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Segunda aresta BC
  teste_aresta(P[1],N[1],N[2],sgn0,ng0,NG,NL,border,Ng,nel,1,vert,0);
  sinal[1]=sgn0;
  imin=imax+1;
  imax=imin+P[1]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta AC
  teste_aresta(P[1],N[0],N[2],sgn0,ng0,NG,NL,border,Ng,nel,2,vert,0);
  sinal[2]=sgn0;
  imin=imax+1;
  imax=imin+P[1]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta AD
  teste_aresta(P[2],N[0],N[3],sgn0,ng0,NG,NL,border,Ng,nel,3,vert,0);
  sinal[3]=sgn0;
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta BD
  teste_aresta(P[2],N[1],N[3],sgn0,ng0,NG,NL,border,Ng,nel,4,vert,0);
  sinal[4]=sgn0;
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
  // Aresta CD
  teste_aresta(P[2],N[2],N[3],sgn0,ng0,NG,NL,border,Ng,nel,5,vert,0);
  sinal[5]=sgn0;
  imin=imax+1;
  imax=imin+P[2]-2;
  map_aresta(imin,imax,sgn0,ng0,map,sgn);
