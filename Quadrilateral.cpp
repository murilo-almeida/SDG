...
// ****************************************************************************
/*
void Quadrilateral::Processar_geometria(int nel,
					const Vertice * vert,
					const int numv, 
					const int * VN,
					int map[],
					...
					int f_mask[], const int N_add)
{
	//printf("Quadrilateral::Processar_geometria\n");
  int n0,n1,n2,n3,sgn0,sgn1,sgn2,sgn3,ng0,ng1,ng2,ng3;

  //int num,type;
  if(numv != 4){
    printf("Incompatibilidade: Dado do elemento nao eh de Quadrilatero\n");
    exit(0);
  } 
  ...
  n2=VN[2];
  n3=VN[3];
  // ordenacao dos nos: a mesma para o format do Gambit e do Gmsh
  // 3 -------- 2
 //  |          |
 //  |          |
 //  0 -------- 1
 
 // printf("border.size = %ld\n",border.size());
  teste_aresta(P[0],n0,n1,sgn0,ng0,NG,NL,border,Ng,nel,0,vert,-1);
  teste_aresta(P[1],n1,n2,sgn1,ng1,NG,NL,border,Ng,nel,1,vert,-1);
  teste_aresta(P[0],n3,n2,sgn2,ng2,NG,NL,border,Ng,nel,2,vert, 1);
  teste_aresta(P[1],n0,n3,sgn3,ng3,NG,NL,border,Ng,nel,3,vert, 1);
 // printf("Na de border[border.size -1] = %d\n",border[border.size()-1].Na);
  sinal[0]=sgn0;
  sinal[1]=sgn1;
  sinal[2]=sgn2;
  sinal[3]=sgn3;
 ...
 // n2=VN[2]+N_add;
 // n3=VN[3]+N_add;
 // make_gbnmap(n0,n1,n2,n3,ng0,ng1,ng2,ng3,sgn0,sgn1,sgn2,sgn3,map,sgn);
 
}
*/
// ****************************************************************************
/*     OBSOLETA   - Testar sem
void Quadrilateral::make_gbnmap(int n0,int n1,int n2,int n3,
				int ng0,int ng1,int ng2,int ng3,
				int sign0,int sign1,int sign2,int sign3,
				int map[],int sgn[] )
{
 //printf("Quadrilateral::make_gbnmap\n");
  int p=P[0];
  int i;
  int imax,imin;
  int temp;
  
  //printf("SINAIS: %d %d %d %d\n", sign0,sign1,sign2,sign3);
  map[0]=n0; 
  sgn[0]=1;
  map[1]=n1; 
  sgn[1]=1;
  ...
  map[3]=n3; 
  sgn[3]=1;
  //printf("SINAIS de 0 1 2 3: %d %d %d %d\n", sgn[0],sgn[1],sgn[2],sgn[3]);
